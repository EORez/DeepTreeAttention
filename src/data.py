...
def sample_plots(shp, min_train_samples=5, min_test_samples=3, iteration = 1):
    ...
    train = shp.groupby("taxonID").apply(lambda x: x.sample(frac=0.8)).reset_index(drop=True)
    train = train.groupby("taxonID").apply(lambda x: x.head(100)).reset_index(drop=True)
